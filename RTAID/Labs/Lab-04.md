...
7. This shows you the schema (column names and data types) for the table. One thing that will be helpful to add to this table on the KQL Database would be a hidden column for the ingestion time that will be used later in the Medallion architecture. Let's add that now. Copy and paste the script below to alter the tables you just created by adding an ingestion time column.
    ...
    >**Note**: The IngestionTime Policy adds a hidden column (_ingestion_time) to a table, capturing the timestamp when records are ingested into the database. This column is not visible in the physical schema but becomes accessible when querying the table (e.g., SilverAddress table). Its significance lies in tracking data arrival, enabling incremental processing, and supporting Medallion architecture by organizing data based on ingestion time. Similar policies include Retention Policy (managing data lifespan), Update Policy (applying transformations during ingestion), Merge Policy (optimizing data shards), and Partitioning Policy (improving query performance). The policy helps streamline real-time analytics, debugging, and efficient data processing workflows.

9. The four new tables are blank tables with their schema defined. Now you need a way to properly load these tables. Navigate back to your workspace **RTI_<inject key="DeploymentID" enableCopy="false"/>**.
