...
2. **リアルタイムハブへようこそ**ダイアログが開き、**ツアーを見る**または**開始する**を自由に 選択できます。

    ![](../media/Lab-01/image027.jpg)

3. リアルタイムハブは、組織全体にわたって移動するデータをストリーミングするための 1 つの場所です。すべてのMicrosoft Fabric テナントは、自動的にこのハブと共にプロビジョニングされます。これを使えば、さまざまなソースからの移動するデータを簡単に検出、インジェスト、管理、利用することができます。
